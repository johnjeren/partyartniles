...
jQuery(document).ready(function($) {
    ...
    // link to link
  if($("select[name='universal_portf_icon'] option:selected").val() == 'link_to_link') {
    $("#universal_portf_link").show();
  }

    $('select[name="universal_portf_icon"]').on("change", function(event){
      var str = '';
      str = $("select[name='universal_portf_icon'] option:selected").val();

     if(str == 'link_to_link')
         $("#universal_portf_link").fadeIn();
      else
          $("#universal_portf_link").hide();
       
    }); 

    // link to image gallery lightbox
  if($("select[name='universal_portf_icon'] option:selected").val() == 'lightbox_to_gallery') {
    $("#universal_portf_gallery").show();
  }
    ...
    $('select[name="universal_portf_icon"]').on("change", function(event){
     ...
     if(str == 'lightbox_to_gallery')
         $("#universal_portf_gallery").fadeIn();
      else
          $("#universal_portf_gallery").hide();
       
    });   

    // link to video in lightbox
  if($("select[name='universal_portf_icon'] option:selected").val() == 'lightbox_to_video') {
    $("#universal_portf_video").show();
  }
    ...
    $('select[name="universal_portf_icon"]').on("change", function(event){
     ...
     if(str == 'lightbox_to_video')
         $("#universal_portf_video").fadeIn();
      else
          $("#universal_portf_video").fadeOut();
       
    });      


  if($("#universal_portfolio_navigation input[value='no-filter']:checked").val() == 'no-filter') {
    $("#universal_nav_number").show();
  }  

    $('input[name="universal_portfolio_navigation"]').on("change", function(event){
      var str = '';
      str = $("#universal_portfolio_navigation input[value='no-filter']:checked").val();

     if(str == 'no-filter')
         $("#universal_nav_number").fadeIn();
      else
          $("#universal_nav_number").fadeOut();
	...
	jQuery.fn.extend({
	    ShowStyles: function () {
			...
			subheader_styles['style-1'] = "#universal_bg_type, #universal_subheader_img, #universal_subheader_color";
			subheader_styles['style-2'] = "#universal_bg_type, #universal_subheader_img, #universal_subheader_color";
			subheader_styles['style-3'] = "#universal_bg_type, #universal_subheader_img, #universal_subheader_color";
	...
	if ((a == ('template-portfolio.php')) || (a == ('template-gallery.php')) || (a == ('template-homepage.php')) || (a == ('template-onepage.php')) ) {
		...
		jQuery('#page_sidebars #universal_sidebar_position').hide();
	}
	...
	jQuery.fn.extend({
	    ShowPostFormats: function () {
			...
			var post_formats = {};
			post_formats['post-format-0'] = "#universal_standard_post_custom_fields";
			post_formats['post-format-gallery'] = "#universal_gallery_post_custom_fields";
			post_formats['post-format-link'] = "#universal_link_post_custom_fields";
			post_formats['post-format-quote'] = "#universal_quote_post_custom_fields";
			post_formats['post-format-audio'] = "#universal_audio_post_custom_fields";
			post_formats['post-format-video'] = "#universal_video_post_custom_fields";
	...
	jQuery.fn.extend({
	    ShowPageFields: function () {
			...
			//page templates
			var page_templates = {};
			page_templates['template-blog.php'] = "#universal_blog_options";
			page_templates['template-portfolio.php'] = "#universal_portfolio_options";
			page_templates['template-contact.php'] = "#universal_contact_options";
			page_templates['template-onepage.php'] = "#universal_onepage_options";
