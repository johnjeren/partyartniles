<?php
 	$link_post_link_title = get_post_meta($post->ID,'universal_link_title_block',true);
	$link_post_data = get_post_meta($post->ID,'universal_link_block',true);
	$link_post_target = get_post_meta($post->ID,'universal_link_radio',true);
	$link_post_relationship = get_post_meta($post->ID,'universal_link_relationship',true);
...
?>
		...
		$image_url = wp_get_attachment_url($thumbnail_id);		
 		...
 		if(is_single()) {?>
			...
			<div class="link-format"><a href="<?php echo esc_url($link_post_data); ?>"  rel="<?php echo esc_attr($link_post_relationship); ?>"  title="<?php echo esc_attr($link_post_link_title); ?>" target="_<?php echo esc_attr($link_post_target) ?>" ><?php echo esc_attr($link_post_link_title); ?></a></div>
			</div>
			...
			<?php } else { ?>
				<div class="post-thumbnail">
					<div class="post-hover">
						<a href="<?php the_permalink(); ?>" style="background:url(<?php echo esc_url($image[0]) ?>); height: <?php echo esc_attr($image_height) ?>;"></a>
					</div>
				</div>
