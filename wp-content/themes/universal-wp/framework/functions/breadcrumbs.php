...
function universal_breadcrumbs(){
	/* === OPTIONS === */
    $text['home']     = esc_html__( 'Home', 'universal-wp' ); // text for the 'Home' link
    $text['category'] = esc_html__( 'Archive by Category "%s"', 'universal-wp' ); // text for a category page
    $text['search']   = esc_html__( 'Search Results for "%s" Query', 'universal-wp' ); // text for a search results page
    $text['tag']      = esc_html__( 'Posts Tagged "%s"', 'universal-wp' ); // text for a tag page
    $text['author']   = esc_html__( 'Articles Posted by %s', 'universal-wp' ); // text for an author page
    $text['404']      = esc_html__( 'Error 404', 'universal-wp' ); // text for the 404 page
    ...
    if (is_home() || is_front_page()) {
        ...
        if ( get_query_var('paged') ) {
            ...
            echo '<span class="universal_page_bread">'.esc_html__(' - Page','universal-wp') . ' ' . get_query_var('paged').'</span>';
            if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo ')';
        }
...
function universal_crumbs_tax($term_id, $tax, $sep){
	$termlink = array();
	while( (int)$term_id ){
